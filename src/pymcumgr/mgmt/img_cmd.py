...
class CmdImg(CmdBase):
    ...
    # @staticmethod
    # def _encode(hdr, payload_map, seq=0):
    #     payload = CborAttr.encode(payload_map)
    ...
    # @staticmethod
    # def imageList(seq=0):
    #     return CmdImg.getState(seq=seq)

    # @staticmethod
    # def imageListCompleted(rsp):
    #     return CmdImg.getStateCompleted(rsp)
    ...
    def setState(new_state, seq=0):
        ...
        # return CmdImg._encode(hdr, new_state, seq=seq)
    ...
    def getState(seq=0):
        ...
        return CmdImg(hdr, {})
        # return CmdImg._encode(hdr, {}, seq=seq)


    # @staticmethod
    # def imageTest(img_hash, seq=0):
    #     if isinstance(img_hash, str):
    #         if len(img_hash) != 64:
    #             raise ValueError("Wrong hash length")
    #         img_hash_b = bytes([ int(img_hash[idx:idx+2], 16) for idx in range(0, len(img_hash), 2) ])
    #     elif isinstance(img_hash, bytes):
    #         if len(img_hash) != 32:
    #             raise ValueError("Wrong hash length")

    #         img_hash_b = img_hash
    #     assert type(img_hash_b) == bytes
...
class ImageUpload(RequestBase):
    def __init__(self, mcuboot_image, mtu=252, progress=False):
        super().__init__()
        #mcuboot_image = MCUBootImage(mcuboot_image)
    ...
    def parse_response(self, rsp):
        ...
        if self.response_data['off'] != self.next_offset:
            print('Missed a packet, resending offset:', self.response_data['off'])

        self.current_offset = self.response_data['off']

        if self.progress:
            print('{} / {}'.format(self.current_offset, len(self.image)))
